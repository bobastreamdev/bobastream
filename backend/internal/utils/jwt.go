...
func GenerateAccessToken(user *models.User) (string, error) {
	claims := JWTClaims{
		...
		RegisteredClaims: jwt.RegisteredClaims{
			ExpiresAt: jwt.NewNumericDate(time.Now().Add(config.GetAccessTokenDuration())),
...
func GenerateRefreshToken(user *models.User) (string, error) {
	claims := JWTClaims{
		...
		Role:   user.Role,
		RegisteredClaims: jwt.RegisteredClaims{
			ExpiresAt: jwt.NewNumericDate(time.Now().Add(config.GetRefreshTokenDuration())),
			IssuedAt:  jwt.NewNumericDate(time.Now()),
			NotBefore: jwt.NewNumericDate(time.Now()),
		},
	}
	...
	token := jwt.NewWithClaims(jwt.SigningMethodHS256, claims)
	return token.SignedString([]byte(config.GlobalConfig.JWT.RefreshSecret))
}
...
func ValidateRefreshToken(tokenString string) (*JWTClaims, error) {
	token, err := jwt.ParseWithClaims(tokenString, &JWTClaims{}, func(token *jwt.Token) (interface{}, error) {
		...
		return []byte(config.GlobalConfig.JWT.RefreshSecret), nil
	})
